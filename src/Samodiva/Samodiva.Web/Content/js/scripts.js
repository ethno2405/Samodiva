$(window).load(function(){
	var nav=$('nav')
		,content=$('#content')
		,content_inner=$('>.inner',content)
		,content_inner_width=content_inner[0].offsetWidth
		,content_layer_1=$('>.z1',content)
		,content_layer_2=$('>.z2',content)
		,content_layer_3=$('>.z3',content)
		,layer_1_k=1.2
		,layer_2_k=1.4
		,layer_3_k=1.6
		,track=$('#glob>.track')
		,track_a=$('a',track)
		,back_to_home=$('footer a.back_to_home')
		,back_btn=$('footer a.back')
		,track_a_bl=false
		,track_width=track.width()-track_a.width()
		,content_x=0
		,glob_bl=false
	...
	$('.r-menu a')
		.each(function(){
			var th=$(this)
				,img=$('<img>').attr({src:'Content/images/mrk-1.png'})
			th.append(img)
		})
	
	$('.parts').on('mousedown',function(){
			return false
	})
	
	$('.parts').each(function(){
		var th=$(this)
			,left=+(th.prop('offsetLeft')/content_inner_width*100+'').slice(0,4)+'%'		
		th
			.css({
				left:left
			})
	...
	track_a
		.dragg({
			...
			,drag:function(posX){
				...
				var anim=false
				if(content_x>1)
					anim={duration:400,easing:'easeInBack',complete:function(){track_a_bl=false}}
					,track_a_bl=true				
				render_content(content_x=posX,anim)
			}
			,end:function(){
				track_a.trigger('refresh')
				glob_bl=false
			}
		})
	...
	var map=(function(){
		...
		.each(function(){
			var th=$(this)
			,pos=(th.prop('offsetLeft')-(dbow-th.width())/2)/(content_inner_width-dbow)
	...
	function render_content(pos,anim){
		pos||(pos=content_x)		
		...
		var _fu=function(el,k,pos){
				el.css({left:-(content_inner_width*k-document.body.offsetWidth)*pos})
		}
		if(anim){
			anim=$.extend({},anim,{step:function(now){
												var pos=-now/(content_inner_width-document.body.offsetWidth)					
			...
			content_inner
				.stop()
				.animate({
					left:-(content_inner_width-document.body.offsetWidth)*pos
				},anim)
		}else{
			_fu(content_inner,1,pos)
			_fu(content_layer_1,layer_1_k,pos)
			_fu(content_layer_2,layer_2_k,pos)
			_fu(content_layer_3,layer_3_k,pos)
		}		
	...
	content_inner
		.tabs({
			...
			,actFu:function(_){
				...
				if(_.curr)					
					_t=content_x=(_.curr.prop('offsetLeft')-(dbow-_.curr.width())/2)/(content_inner_width-dbow)
					,render_content(_t,anim)
					,track_a
						.stop()
						.animate({
							left:track_width*(_t>1?1:_t)
							},anim)
					,$.when(track_a).then(function(){
							track_a.trigger('refresh')
					})				
	...
	nav
		...
		.navs(function(n,_){
			if(n===_.defHash||n==='close')
				!glob_bl&&render_content(content_x=0,{duration:1000,easing:'easeInOutSine'})
				,!glob_bl&&track_a.stop().animate({left:0},{duration:1000,easing:'easeInOutSine',complete:function(){track_a.trigger('refresh')}})
				,back_btn.fadeOut()
			else
...
;(function($){
	...
	$.fn.dragg=function(o){
		return this.each(function(){
			...
			th.on('refresh',function(){				
				posX=th.prop('offsetLeft')/max
				posY=th.prop('offsetTop')/may
			})
			win.add(body).on(endEv,function(e){
				end(e)
				return false
			})
